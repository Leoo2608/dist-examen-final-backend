...
export const registerEmail=async(req, res)=>{
    try {
        const {destinatario, titulo, texto, idusuario} = req.body;
        const response = await pool.query('select fc_register_email($1,$2,$3,$4)', [destinatario, titulo, texto, idusuario]);
        if(response){
           await sendEmail(destinatario,titulo,texto);
        }
